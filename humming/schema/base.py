"""Base schema classes mirroring upstream humming's schema.base.

The class-level ``WEIGHT_SCHEMA_MAP``/``INPUT_SCHEMA_MAP`` are populated by
``humming.schema.__init__`` exactly like upstream, and ``from_config`` maps
config dict keys onto dataclass field names (with per-class ``KWARGS_ALIAS``).
Only the indexed W4A16 scope is backed by a working kernel; other quant
methods raise at conversion time.
"""
...
class BaseWeightSchema:
    ...
    def may_add_expert_dim(
        ...
        for attr in tensors_attrs.values():
            if attr["shape"] == (1,):
    ...
    def validate_tensors(
        ...
        for key, attr in tensors_attrs.items():
            ...
            tensor = tensors[key]
            assert attr["dtype"] == tensor.dtype, (
                f"{key} dtype mismatched: {attr['dtype']=}, {tensor.dtype=}"
            )
            assert attr["shape"] == tensor.shape, (
                f"{key} shape mismatched: {attr['shape']=}, {tensor.shape=}"
            )
    ...
    def from_config(cls, config: dict[str, Any]) -> "BaseWeightSchema":
        ...
        if cls is BaseWeightSchema:
            ...
            if quant_method in WEIGHT_SCHEMA_MAP:
                return WEIGHT_SCHEMA_MAP[quant_method].from_config(config)
            raise NotImplementedError(
                "the chord humming shim implements indexed W4A16 only; "
                f"weight quant_method={quant_method!r} is not supported"
            )
        ...
        for field in dataclasses.fields(cls):
            if field.name in config:
                ...
                continue
            for alias_name in cls.KWARGS_ALIAS.get(field.name, []):
                if alias_name in config:
                    kwargs[field.name] = config[alias_name]
                    break
...
class BaseInputSchema:
    ...
    def get_fallback_input_dtype(
        ...
        """The indexed W4A16 kernel consumes BF16 activations as-is.

        16-bit inputs pass through; anything sub-16-bit reports the kernel's
        BF16 fallback instead of silently quantizing (the kernel rejects an
        input scale at launch, which is the fail-closed behavior).
        """
    ...
    def from_config(cls, config: dict[str, Any]) -> "BaseInputSchema":
        ...
        if cls is BaseInputSchema:
            ...
            if quant_method in INPUT_SCHEMA_MAP:
                return INPUT_SCHEMA_MAP[quant_method].from_config(config)
            raise NotImplementedError(
                "the chord humming shim implements BF16 activations only; "
                f"input quant_method={quant_method!r} is not supported"
            )
...
__all__ = ["BaseInputSchema", "BaseWeightSchema"]
